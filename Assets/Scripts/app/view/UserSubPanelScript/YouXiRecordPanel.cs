...
public class YouXiRecordPanel : UserSubPanelBase
{
    public const byte byRord = 33;
	//返回
    ...
    public ShaiXuanYouXiRecordPanel shaixuanpanel;
    //新加的
    public UIGrid grid;
	...
	public void show(RecordBackObj obj)
	{
		...
		{
			YouXiRecordItem script = itemGameObjs[i].GetComponent<YouXiRecordItem>();
	...
	void OnNext(GameObject go)
	{
		...
		if (_RecordBackObj.iCuePage < totalPages) {
			...
			NetworkManager.Instance.LookupRecord(byRord,(byte)page, byRord, page, YouXiRecordPanel.chName, YouXiRecordPanel.startDate, YouXiRecordPanel.endDate);
			ProcessGameObjs (0);
		} else {
	...
	void ProcessGameObjs(int count)
	{
		...
		if (itemGameObjs.Count < count)
		{
			GameObject asset = (GameObject)AssetManager.Instance.LoadPrefab("newpanels/YouXiRecordItem");
			for (int i = 0; i < n; ++i)
			{
