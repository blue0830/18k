...
public class TouZhuRecordPanel : UserSubPanelBase
{
    public const byte byRord = 1;
	//返回
	...
	public ShaiXuanTouZhuRecordPanel shaixuanpanel;
    ...
    public void show(RecordBackObj obj)
	{
        ...
        {
            TouZhuRecordItem script = itemGameObjs[i].GetComponent<TouZhuRecordItem>();
    ...
    void ProcessGameObjs(int count)
    {
		...
		if (itemGameObjs.Count < count)
        {
            GameObject asset = (GameObject)AssetManager.Instance.LoadPrefab("newpanels/TouZhuRecordItem");
            for (int i = 0; i < n; ++i)
            {
	...
	void OnNext(GameObject go)
	{
        ...
        if (_RecordBackObj.iCuePage < totalPages) {
			...
			NetworkManager.Instance.LookupRecord(byRord,(byte)page, byRord, page, TouZhuRecordPanel.chName, TouZhuRecordPanel.startDate, TouZhuRecordPanel.endDate);
			ProcessGameObjs (0);
		} else {
