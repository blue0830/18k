...
//会员资料
public class XiaJiYingKuiMingXIPanel : UserSubPanelBase {
    ...
    public ShaiXuanXiaJiYingKuiMingXi shaixuanpanel;
    ...
    public void show(RecordBackObj obj)
    {
        ...
        {
            XiaJiYingKuiMingXiItem script = itemGameObjs[i].GetComponent<XiaJiYingKuiMingXiItem>();
    ...
    void ProcessGameObjs(int count)
    {
        ...
        if (itemGameObjs.Count < count)
        {
            GameObject asset = (GameObject)AssetManager.Instance.LoadPrefab("subPanelMember/Items/XiaJiYingKuiMingXiItem");
            for (int i = 0; i < n; ++i)
            {
    ...
    void OnNext(GameObject go)
    {
        ...
        if (_RecordBackObj.iCuePage < totalPages)
        {
            ...
            NetworkManager.Instance.LookupRecord(byRord, (byte)page, byRord, page, XiaJiYingKuiMingXIPanel.chName, XiaJiYingKuiMingXIPanel.startDate, XiaJiYingKuiMingXIPanel.endDate, XiaJiYingKuiMingXIPanel.lookuserId);
            ProcessGameObjs(0);
        }
        else
        {
