...
//会员资料
public class XiaJiTiXianPanel : UserSubPanelBase {

    public const byte byRord = 21;
    //返回
    ...
    public ShaiXuanXiaJiTiXian shaixuanpanel;
    ...
    public void show(RecordBackObj obj)
    {
        ...
        {
            XiaJiTiXianItem script = itemGameObjs[i].GetComponent<XiaJiTiXianItem>();
    ...
    void ProcessGameObjs(int count)
    {
        ...
        if (itemGameObjs.Count < count)
        {
            GameObject asset = (GameObject)AssetManager.Instance.LoadPrefab("subPanelMember/Items/XiaJiTiXianItem");
            for (int i = 0; i < n; ++i)
            {
    ...
    void OnNext(GameObject go)
    {
        ...
        if (_RecordBackObj.iCuePage < totalPages)
        {
            ...
            NetworkManager.Instance.LookupRecord(byRord, (byte)page, byRord, page, XiaJiTiXianPanel.chName, XiaJiTiXianPanel.startDate, XiaJiTiXianPanel.endDate, XiaJiTiXianPanel.lookuserId);
            ProcessGameObjs(0);
        }
        else
        {
