...
//会员资料
public class ZhuanZhangJiLuPanel : UserSubPanelBase {

    public const byte byRord = 8;
    //返回
    ...
    public ShaiXuanZhuanZhangJiLu shaixuanpanel;
    ...
    void ProcessGameObjs(int count)
    {
        ...
        if (itemGameObjs.Count < count)
        {
            GameObject asset = (GameObject)AssetManager.Instance.LoadPrefab("subPanelMember/Items/ZhuanZhangJiLuItem");
            for (int i = 0; i < n; ++i)
            {
    ...
    void OnNext(GameObject go)
    {
        ...
        if (_RecordBackObj.iCuePage < totalPages)
        {
            ...
            NetworkManager.Instance.LookupRecord(byRord, (byte)page, byRord, page, ZhuanZhangJiLuPanel.chName, ZhuanZhangJiLuPanel.startDate, ZhuanZhangJiLuPanel.endDate, ZhuanZhangJiLuPanel.lookuserId);
            ProcessGameObjs(0);
        }
        else
        {
