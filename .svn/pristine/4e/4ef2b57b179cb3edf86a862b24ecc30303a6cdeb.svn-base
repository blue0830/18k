...
public class QuKuanRecordbkPanel : UserSubPanelBase
{
    public const byte byRord = 7;
    //返回
    ...
    public ShaiXuanQuKuanPanel shaixuanpanel;
	...
	public void show(RecordBackObj obj)
    {
		...
		{
			QuKuanRecordItem script = itemGameObjs[i].GetComponent<QuKuanRecordItem>();
    ...
    void OnNext(GameObject go)
    {
        ...
        if (_RecordBackObj.iCuePage < totalPages) {
			...
			NetworkManager.Instance.LookupRecord(byRord,(byte)page, byRord, page, QuKuanRecordbkPanel.chName, QuKuanRecordbkPanel.startDate, QuKuanRecordbkPanel.endDate);
			ProcessGameObjs (0);
		} else {
	...
	void ProcessGameObjs(int count)
	{
		...
		if (itemGameObjs.Count < count)
		{
			GameObject asset = (GameObject)AssetManager.Instance.LoadPrefab("newpanels/QuKuanRecordItem");
			for (int i = 0; i < n; ++i)
			{
