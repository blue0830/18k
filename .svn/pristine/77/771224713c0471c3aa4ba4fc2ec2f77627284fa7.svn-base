...
//会员资料
public class XiaJiYouXiJiLuPanel : UserSubPanelBase {

    public const byte byRord = 36;
    //返回
    ...
    public ShaiXuanXiaJiYouXiJiLu shaixuanpanel;
    ...
    public void show(RecordBackObj obj)
    {
        ...
        {
            XiaJiYouXiJiLuItem script = itemGameObjs[i].GetComponent<XiaJiYouXiJiLuItem>();
    ...
    void ProcessGameObjs(int count)
    {
        ...
        if (itemGameObjs.Count < count)
        {
            GameObject asset = (GameObject)AssetManager.Instance.LoadPrefab("subPanelMember/Items/XiaJiYouXiJiLuItem");
            for (int i = 0; i < n; ++i)
            {
    ...
    void OnNext(GameObject go)
    {
        ...
        if (_RecordBackObj.iCuePage < totalPages)
        {
            ...
            NetworkManager.Instance.LookupRecord(byRord, (byte)page, byRord, page, XiaJiYouXiJiLuPanel.chName, XiaJiYouXiJiLuPanel.startDate, XiaJiYouXiJiLuPanel.endDate, XiaJiYouXiJiLuPanel.lookuserId);
            ProcessGameObjs(0);
        }
        else
        {
