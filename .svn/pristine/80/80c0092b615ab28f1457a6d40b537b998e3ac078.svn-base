...
//会员资料
public class XiajiTouZhuMingXiPanel : UserSubPanelBase {

    public const byte byRord = 30;
    //返回
    ...
    public ShaiXuanXiaJiTouZhuMingXi shaixuanpanel;
    ...
    public void show(RecordBackObj obj)
    {
		...
		int length = labelstrs.Length / col-2;
        ...
        {
            XiaJiTouZhuMingXiItem script = itemGameObjs[i].GetComponent<XiaJiTouZhuMingXiItem>();
    ...
    void ProcessGameObjs(int count)
    {
        ...
        if (itemGameObjs.Count < count)
        {
            GameObject asset = (GameObject)AssetManager.Instance.LoadPrefab("subPanelMember/Items/XiaJiTouZhuMingXiItem");
            for (int i = 0; i < n; ++i)
            {
    ...
    void OnNext(GameObject go)
    {
        ...
        if (_RecordBackObj.iCuePage < totalPages)
        {
            ...
            NetworkManager.Instance.LookupRecord(byRord, (byte)page, byRord, page, XiajiTouZhuMingXiPanel.chName, XiajiTouZhuMingXiPanel.startDate, XiajiTouZhuMingXiPanel.endDate, XiajiTouZhuMingXiPanel.lookuserId);
            ProcessGameObjs(0);
        }
        else
        {
