...
//会员资料
public class HuiYuanZiLiaoPanel : UserSubPanelBase {

    public const byte byRord = 2;
    //返回
    ...
    public ShaiXuanHuiYuanZiLiao shaixuanpanel;

    public UITextList cMember;
    public UILabel xiaxianNo;
    public UILabel zaixianNo;
    ...
    public FanDianPanel fandianPanel;
    public PeiEPanel peiEPanel;
    ...
    void Showfandian(uint id, string name, string fandian)
    {
        fandianPanel.show(id, name, fandian);
    }

    public void ShowPeiE(MSG_GP_USER_GETPLAYPE para)
    {
        peiEPanel.Show(para);
    }
    ...
    public void show(RecordBackObj obj)
    {
        ...
        {
            HuiYuanZiLiaoItem script = itemGameObjs[i].GetComponent<HuiYuanZiLiaoItem>();
            script.FillData(dataObjs[i], Showfandian);
        }
    ...
    void ProcessGameObjs(int count)
    {
        ...
        if (itemGameObjs.Count < count)
        {
            GameObject asset = (GameObject)AssetManager.Instance.LoadPrefab("subPanelMember/Items/HuiYuanZiLiaoItem");
            for (int i = 0; i < n; ++i)
            {
                GameObject go = Instantiate(asset) as GameObject;
